process SUMMARY {
    ...
    # F13L coverage
    pct_F13L_covered=\$(samtools coverage -r ${params.genomic_region} ${map_ref_bam} | awk 'NR>1' | cut -f6)
    pct_F13L_covered_formatted=\$(printf "%.2f" "\${pct_F13L_covered}")
    mean_F13L_coverage=\$(samtools coverage -r ${params.genomic_region} ${map_ref_bam} | awk 'NR>1' | cut -f7)
    mean_F13L_coverage_formatted=\$(printf "%.2f" "\${mean_F13L_coverage}")
    num_bases_F13L_50x=\$(samtools depth -r ${params.genomic_region} ${map_ref_bam} | awk '{if(\$3>50)print\$3}' | wc -l)
    pct_F13L_50x=\$(echo "\${num_bases_F13L_50x}/${gene_length}*100" | bc -l)
    pct_F13L_50x_formatted=\$(printf "%.2f" "\${pct_F13L_50x}")
    num_bases_F13L_100x=\$(samtools depth -r ${params.genomic_region} ${map_ref_bam} | awk '{if(\$3>100)print\$3}' | wc -l)
    pct_F13L_100x=\$(echo "\${num_bases_F13L_100x}/${gene_length}*100" | bc -l)
